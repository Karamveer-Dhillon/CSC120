...
int main() {
  ...
  do {
    ...
    // If the input is not a number, it will clear the error and ignore the invalid input.
    while (cin.fail()){
      cin.clear();
    ...
    // If the input is not equal to the exit code, it will add the number of books
    // to the total number of books and increment the number of students.
  ...
  // Checked if the number of students is 0. If it is, it will display a message and exit the program.
  if (numStudents == 0) {
    ...
    // Displayed the total number of students, total number of books bought by all students,
    // and the average number of books bought by each student.
    cout << "\nSales Report\n";
